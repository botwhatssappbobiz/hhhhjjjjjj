...
if (m.isOwner) return m.reply(bvl)
	...
	sock.sendMessage(m.cht, {text:`Woi Kontol Gausah Promosi Link Anjg Kek Punya Lu Aja Nih Grup😹`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
  }
}
...
if (m.isOwner) return m.reply(bvl)
    	...
    	sock.sendMessage(m.cht, {text:`Toxic Amat Lo Rek😹`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
	}
 }
   ...
   case "tourl": {   
    ...
    m.react("🕙")
     if (/image|video|webp/.test(quoted.msg.mimetype)) {
      ...
      const cat = await catbox(media)
      m.reply(cat)
     } else if (/audio|document/.test(quoted.msg.mimetype)) {
    const { catbox } = require('../lib/uploader.js')
      const media = await m.quoted.download()
      const cat = await catbox(media)
      m.reply(cat)
     } else {
      m.reply('reply media anda mau di url')
     }
    break
   }
